...
psr = psr[(psr["bistable"] == True) & (psr["switch"] == True) & (psr["resettable"] == True) & (psr["sound"] == True)].reset_index(drop=True)
trunc = min(psr.shape[0], trunc_upper)

psr = psr.drop(["switch", "bistable", "resettable", "sound", "on_thresh", "d_thresh", "off_SS"], axis=1)
psr = psr[:trunc]
